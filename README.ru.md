...
Если yt-dlp отвечает ошибкой `The web client only works when logged-in`, Napotom один раз пробует получить видео через встроенный плеер Vimeo. Ссылку вручную менять не нужно: сначала приложение всегда использует исходный адрес и сохраняет ключ доступа для видео по ссылке.

Если повторная попытка не помогла, приложение покажет причину ошибки и предложит добавить **cookies Vimeo** в настройках. Для видео с ограниченным доступом авторизация по-прежнему может требоваться. Обновляйте yt-dlp через **Настройки → yt-dlp**: новые версии могут исправлять совместимость с сайтами.
